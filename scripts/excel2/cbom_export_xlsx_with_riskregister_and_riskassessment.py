...
SHEET_CBOM = "2_CBOM"
SHEET_RISK_REGISTER = "3_RiskRegister"
SHEET_RISK_ASSESSMENT = "4_RiskAssessment"
...
CONTROL_DEFAULT = "Standard IT Security Controls"
...
# --- Risk Register mapping (sheet 3) ---
def classify_register_criticality_and_risk(cbom: dict) -> tuple[str, str]:
    algo = (cbom.get("algorithm_used") or "").lower()
    cat = (cbom.get("algorithm_category") or "").lower()
...
# --- Risk Assessment mapping (sheet 4) ---
def pqc_threat(algo: str) -> tuple[str, str]:
    # Returns (Risk, Cause)
    a = (algo or "").lower()

    if any(x in a for x in ["rsa", "ecd", "ecdh", "ecdsa", "dsa", "dh", "ed25519", "ed448"]):
        return (
            "Exposure to Shor's Algorithm (Quantum Integer Factorization / Discrete Log)",
            f"Usage of Asymmetric algorithm ({algo}) which is not quantum-resistant",
        )

    return (
        "Exposure to Grover's Algorithm (Quantum Search)",
        f"Usage of algorithm ({algo}) which is not quantum-resistant",
    )
...
def main():
    ...
    # -------------------------
    ws_rr = wb.create_sheet(SHEET_RISK_REGISTER)
    ws_rr.append(RISK_REGISTER_HEADERS)
    style_header(ws_rr)

    dedup_rr = OrderedDict()
    for finding in findings:
        ...
        if key not in dedup_rr:
            criticality, risk_text = classify_register_criticality_and_risk(cbom)
            dedup_rr[key] = {
                "system": system_name,
    ...
    for idx, item in enumerate(dedup_rr.values(), start=1):
        ws_rr.append([
            idx,
            item["system"],
            item["asset_type"],
    ...
    autosize_columns(ws_rr, max_width=90)

    # -------------------------
    # Sheet 4_RiskAssessment (derived from Risk Register)
    # One row per (system, algorithm) (deduped)
    # -------------------------
    ws_ra = wb.create_sheet(SHEET_RISK_ASSESSMENT)
    ws_ra.append(RISK_ASSESSMENT_HEADERS)
    style_header(ws_ra)

    dedup_ra = OrderedDict()
    for item in dedup_rr.values():
        system_name = item["system"]
        algo = item["algo"]

        k = f"{system_name}||{algo}".lower().strip()
        if k in dedup_ra:
            continue

        risk_title, risk_cause = pqc_threat(algo)

        # Defaults matching your example
        if "shor" in risk_title.lower():
            impact, likelihood = 5, 5
            mitigation = "Migrate to NIST PQC Standards (ML-KEM/ML-DSA)"
        else:
            impact, likelihood = 3, 3
            mitigation = "Increase Security Level: Double Key Length / Digest Size where applicable (e.g., AES-128 -> AES-256)"
    ...
    for idx, v in enumerate(dedup_ra.values(), start=1):
        ws_ra.append([
            idx,
            v["system"],
            v["algo"],
    ...
    autosize_columns(ws_ra, max_width=95)

    wb.save(OUTPUT_FILE)
    print(f"[+] Exported: {OUTPUT_FILE}")
    print(f"[+] Sheets: {SHEET_CBOM}, {SHEET_RISK_REGISTER}, {SHEET_RISK_ASSESSMENT}")
    print(f"[+] 2_CBOM rows: {len(cbom_rows)}")
    print(f"[+] 3_RiskRegister rows (deduped): {len(dedup_rr)}")
    print(f"[+] 4_RiskAssessment rows (deduped): {len(dedup_ra)}")
